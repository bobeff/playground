...
the library. Your library should offer the following functions:
    ...
    Use upvalue to keep the transliteration table.
*/
...
static const struct luaL_Reg ex5mod[] =
{
  { "settrans", l_settrans },
  { "gettrans", l_gettrans },
...
int LUA_EXPORT luaopen_ex5mod(lua_State *L)
{
  luaL_newlibtable(L, ex5mod);
  lua_newtable(L);
  lua_newtable(L);
  lua_setfield(L, -2, TABLE_INDEX);
  luaL_setfuncs(L, ex5mod, 1);
  return 1;
}
